...
class Application_Model_DbTable_Cmarc4structure extends Zend_Db_Table_Abstract
{

    protected $_name = 'cmarc4structure';
	...
	$select = $this->select();
	$select->order(array('mandatory DESC', 'tagfield ASC', 'tagsubfield asc' ));
	return $this->fetchAll($select);
    }
    ...
    public function getDesc($tagfield, $tagsubfield){
	$select = $this->select();
	$select->where('tagfield = ?', $tagfield);
	$select->where('tagsubfield = ?', $tagsubfield);
	$row = $this->fetchRow($select);
